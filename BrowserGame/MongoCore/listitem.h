...
public:
    ...
    [[deprecated("dont use anymore. will be removed")]]
    ListItem() : DB(MongoCore::instance()->db()),__limit(20),__skip(0){}
    ...
    inline std::vector<T>& UpdateList( const T& filter , const FindOptions& options )
    {
    ...
    inline bool DeleteItem( const T& item ){
        ...
        if( ! item.oid() ) {
            setLastError( "Item must have oid" );
            return false;
        }
    ...
    inline std::vector<T>& List( const T& filter , const FindOptions& options = FindOptions() )
    {
