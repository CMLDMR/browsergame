...
{
    if( ! mDB ) {
        setLastError( QObject::tr("Mongo did not initialized. Please connect to db!").toStdString() );
    }
}
...
uint64_t MongoCore::DB::gridFsFileSize(const std::string &fileOid)
{
    auto bucket = this->db ()->gridfs_bucket ();

    mongocxx::gridfs::downloader downloader;
    try {
        auto roid = bsoncxx::types::bson_value::value(bsoncxx::oid{fileOid});
        downloader = bucket.open_download_stream(roid);
    ...
    return downloader.file_length();
}

std::string MongoCore::DB::downloadFileWeb(const std::string &fileOid, bool forceFilename)
{
    if( ! mDB ) {
        errorOccured( QObject::tr("Mongo did not initialized. Please connect to db!").toStdString() );
        return "";
    }
    std::string file__Name{""};
    bool fileDownloadedBefore=true;
...
bsoncxx::types::bson_value::value MongoCore::DB::uploadfile(const std::string &filepath)
{
    if( ! mDB ) {
        errorOccured( QObject::tr("Mongo did not initialized. Please connect to db!").toStdString() );
        return bsoncxx::types::bson_value::value( bsoncxx::v_noabi::type::k_undefined );
    }
...
mongocxx::stdx::optional<mongocxx::result::update> MongoCore::DB::updateItem(const Item &item)
{
    ...
    if( _errorOccured ) return std::nullopt;
